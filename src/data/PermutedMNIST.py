...
def get_permuted_MNIST(path, batch_size,train):
    ...
    if(train):
        set = ClassIncremental(
            MNIST(data_path="./src/data/MNIST", download=True, train=True),
         increment=2
        )

    else:
        set = ClassIncremental(
            MNIST(data_path="./src/data/MNIST", download=True, train=False),
         increment=2
        )
