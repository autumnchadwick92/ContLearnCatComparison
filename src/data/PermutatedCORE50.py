...
def get_permuted_CORE50(path, batch_size,train):
    ...
    if(train):
        set = ClassIncremental(
            Core50(data_path="./src/data/Core50", download=True, train=True),
         increment=2
        )

    else:
        set = ClassIncremental(
            Core50(data_path="./src/data/Core50", download=True, train=False),
         increment=2
        )
